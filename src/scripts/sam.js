 ...
 *
 * Modified for TardQuest by MySpace Tom
 */
(function (global, factory) {
     ...
     * @return {Promise}
     */
    function Play(context, audiobuffer, opts) {
        return new Promise(function (resolve) {
            ...
            dryGain.gain.value = typeof opts.reverb.dryGain === "number" ? opts.reverb.dryGain : 0.7;
            ...
            if (opts?.reverb) {
                ...
                wetGain.gain.value = typeof opts.reverb.wetGain === "number" ? opts.reverb.wetGain : 0.5;

                const { input: reverbInput, output: reverbOutput } = createSimpleReverb(context, opts.reverb);
     ...
     * @param {Number}  [options.delayTimeSeconds]      Base delay time. Default 0.03.
     * @param {Number}  [options.decay]                 Feedback decay factor. Default 0.5.
     * @param {Number}  [options.totalDelays]           Number of delay taps. Default 3.
     * @param {Number}  [options.lowpassFilterCutoffHz] Lowpass filter cutoff frequency. Default 1000.
     *
     * @return {Promise}
     */
    function createSimpleReverb(context, options = {}) {
        const {
            delayTimeSeconds = 0.03,
            decay = 0.5,
            totalDelays = 3,
            lowpassFilterCutoffHz = 1000,
        } = options;
        ...
        for (let i = 0; i < totalDelays; i++) {
            ...
            delay.delayTime.value = delayTimeSeconds * (i + 1);
            feedback.gain.value = decay / (i + 1);
            filter.type = "lowpass";
            filter.frequency.value = lowpassFilterCutoffHz;
     ...
     * @return {Promise}
     */
    function PlayBuffer(audiobuffer, opts) {
        if (null === context) {
        ...
        return Play(context, audiobuffer, opts);
    }
